...
describe("itemAssignmentReducer", () => {
  it("ASSIGN places the item and sends it to the end", () => {
    const next = itemAssignmentReducer(initial, { type: "ASSIGN_ITEM", itemId: "a" });

    expect(next.map((i) => i.id)).toEqual(["b", "a"]);
    expect(next.at(-1)).toMatchObject({ id: "a", categoryAssignmentStatus: CategoryAssignmentStatus.Assigned });
  });

  it("RETURN places the item back to staging at the end", () => {
    const assigned = itemAssignmentReducer(initial, { type: "ASSIGN_ITEM", itemId: "a" });
    const returned = itemAssignmentReducer(assigned, { type: "UNASSIGN_ITEM", itemId: "a" });

    expect(returned.at(-1)).toMatchObject({ id: "a", categoryAssignmentStatus: CategoryAssignmentStatus.Unassigned });
  });
